"""
Benchmark: Run interaction mining pipeline on Adult Census Income dataset.
"""
...
RESULTS_DIR = Path("data") / "benchmark_adult"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
logger = lambda msg: print(f"[ADULT] {msg}")
...
def load_adult_data():
    ...
    df = pd.read_csv(url, header=None, names=names, na_values=" ?", skipinitialspace=True)
...
def run_interaction_mining_fast(df, outcome_col="income_binary"):
    ...
    rdf.to_csv(RESULTS_DIR / "adult_interaction_rankings.csv", index=False)
    if len(rdf):
        t = rdf.iloc[0]
...
def run_models(df, outcome_col="income_binary"):
    logger("Running predictive models with 5-fold CV...")
    int_path = RESULTS_DIR / "adult_interaction_rankings.csv"
    ...
    rdf.to_csv(RESULTS_DIR / "adult_model_metrics.csv", index=False)
    return rdf


def main():
    logger("=" * 60)
    logger("Adult Census Benchmark — Interaction Mining")
    logger("=" * 60)

    df = load_adult_data()
    df = engineer_features(df)
    logger(f"Engineered: {df.shape[1]} total cols, target mean={df['income_binary'].mean():.3f}")
    ...
    with open(RESULTS_DIR / "adult_summary.json", "w") as f:
