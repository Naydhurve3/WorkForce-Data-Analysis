"""
Benchmark: Run interaction mining pipeline on Bank Marketing dataset.
"""
...
RESULTS_DIR = Path("data") / "benchmark_bank"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
logger = lambda msg: print(f"[BANK] {msg}")
...
def run_interaction_mining_fast(df, outcome_col="deposit"):
    ...
    rdf.to_csv(RESULTS_DIR / "bank_interaction_rankings.csv", index=False)
    if len(rdf):
        t = rdf.iloc[0]
...
def run_models(df, outcome_col="deposit"):
    logger("Running predictive models with 5-fold CV...")
    int_path = RESULTS_DIR / "bank_interaction_rankings.csv"
    ...
    rdf.to_csv(RESULTS_DIR / "bank_model_metrics.csv", index=False)
    return rdf


def main():
    logger("=" * 60)
    logger("Bank Marketing Benchmark — Interaction Mining")
    logger("=" * 60)

    df = load_bank_data()
    df = engineer_features(df)
    logger(f"Engineered: {df.shape[1]} total cols, target mean={df['deposit'].mean():.3f}")
    ...
    with open(RESULTS_DIR / "bank_summary.json", "w") as f:
